class PlayState extends Phaser.State {
  create() {
    console.log('[play] starting play state'); // for easy access to this state for debugging in browser console
    ...
    this.transportSpawnPoints = this.generateCirclePoints(20, 100); // Generate the points that column barrages will come from

    this.columnBarrageSpawnPoints = this.generateCirclePoints(36, config.CANVAS_HYPOT / 2); // Keep track of current barrage

    this.barrage = new Barrage();
    this.timeToNextBarrage = config.MAX_TIME_BETWEEN_BARRAGE;
    this.barrageFunctions = [this.createSpiralBarrage, this.createZigZagBarrage, this.createColumnBarrage]; // here's the big one text!
    ...
    this.game.time.events.add(800, () => {
      ...
      this.game.add.tween(bigOne).to({
        alpha: 1.0
      }, 1200, Phaser.Easing.Quadratic.In, true, 0, 0, true);
  ...
  render() {// this.game.debug.body(this.actors.earth);
  ...
  shutdown() {// _.forEach(this.sounds, sound => sound.stop());
  }
  /* create functions */
  ...
  createCelestial(type) {
    ...
    celestial.body.setCircle(celestial.width / 2); // send the asteroid toward earth

    Phaser.Point.subtract(this.actors.earth.position, celestial.position, celestial.body.velocity);
    celestial.body.velocity.normalize().setMagnitude(speed); // make it spin, so gently
  ...
  createSpiralBarrage(count = 30, radius = config.CANVAS_HYPOT / 2, width = 360, offset = 0, reverse = false, createCelestialCallback = this.createAsteroid.bind(this), delayOffset = 0) {
    ...
    let last = false; // spawn asteroids in a spiral pattern
  ...
  createBarrier() {
    const barrier = this.game.add.sprite(this.game.world.centerX, this.game.world.centerY, 'barrier'); // barrier.scale.set(10, 10);
  ...
  createPerfectNote(x = this.game.world.centerX, y = this.game.world.centerY) {
    ...
    perf.scale.set(0, 0);
    const appearTween = this.game.add.tween(perf.scale).to({
      x: 0.8,
      y: 0.8
    ...
    const disappearTween = this.game.add.tween(perf).to({
      alpha: 0.0
    }, 100, Phaser.Easing.Linear.None, false);
    const disappearTween2 = this.game.add.tween(perf.scale).to({
      x: 1.1,
      y: 1.1
    }, 100, Phaser.Easing.Linear.None, false);
  ...
  updateTouchControls() {
    const {
      x,
      y
    } = this.touchControl.speed;
    const angle = -Math.PI / 2 + this.game.physics.arcade.angleToXY(this.game.world, x, y); // only update the barrier position if the virtual joystick is being
  ...
  updateCelestial(cel) {
    if (!cel.data.isBarrage) {// accelerate single asteroids toward earth
  ...
  asteroidStrike(earth, asteroid) {
    ...
    const sound = this.sounds.AsteroidHit.play(); // If this was from a barrage then log not perfect
    ...
    burstEmitter.makeParticles('asteroid-boom-sheet', new Array(64).fill(0).map((v, i) => i), 64, false, false);
    ...
    const destroyTween = this.game.add.tween(burstEmitter).to({
      alpha: 0.0
  ...
  cometStrike(earth, comet) {
    ...
    this.sounds.CometHit.play(); // If this was from a barrage then log not perfect
    ...
    burstEmitter.makeParticles('comet-boom-sheet', new Array(256).fill(0).map((v, i) => i), 256, false, false);
    burstEmitter.gravity = 0;
    burstEmitter.minParticleSpeed.setTo(-200, -200);
    burstEmitter.maxParticleSpeed.setTo(200, 200);
    burstEmitter.particleDrag.set(100, 100);
  ...
  destroyCelestial(cel, fromBoom = false) {
    ...
    if (fromBoom) {
      ...
      this.game.add.tween(cel.scale).to({
        x: 1.5,
        y: 1.5
      }, 315, Phaser.Easing.Linear.None, true);
    } else {
    ...
    destroyTween.onComplete.add(() => cel.destroy(), this); // check for perfect barrage block
  ...
  launchTransport() {
    if (this.isAlive()) {
      ...
      this.game.add.tween(transport.scale).to({
        x: 1,
        y: 1
      ...
      this.actors.transports.add(transport); // play engine fire animation

      transport.animations.add('afterburner');
      transport.animations.play('afterburner', 20, true); // set a direction away from the center
      ...
      transport.body.velocity.set(-direction.x, -direction.y); // rotate the ship so it's pointing in the right direction
      ...
      transport.rotation += Math.PI / 2; // Set acceleration
  ...
  fireBarrage() {
    ...
    let barrageDuration; // reset current barrage for perfect tracking
  ...
  fireMissile({
    ...
    if (this.isAlive()) {
      ...
      const enlarge = this.game.add.tween(missile.scale).to({
        x: 1,
        y: 1
      }, 1000, Phaser.Easing.Linear.None, true);
      const trajectory = this.game.add.tween(missile.position).to({
        x,
        y
  ...
  explodeMissile(missile) {
    ...
    boom.scale.set(0, 0);
    const tweenDur = 120;
    const boomAlphaTween = this.game.add.tween(boom).to({
      alpha: 0.5,
      tint: 0xFF1010
    }, tweenDur, Phaser.Easing.Cubic.InOut, true, 0, 0, true);
    const boomBodyTween = this.game.add.tween(boom.body).to({
      radius: boom.body.radius * 2.1
    }, tweenDur, Phaser.Easing.Cubic.InOut, true, 0, 0, true);
    const boomTween = this.game.add.tween(boom.scale).to({
      x: 2.5,
      y: 2.5
    }, tweenDur, Phaser.Easing.Cubic.InOut, true, 0, 0, true);
  ...
  damageEarth(dmg) {
    ...
    if (!this.actors.earth.data.beingHit) {
      const hitTween = this.game.add.tween(this.actors.earth).to({
        tint: 0xFF7007
  ...
  blowUpEarth() {
    ...
    const fadeTween = this.game.add.tween(this.actors.earth).to({
      alpha: 0
    }, 3600, Phaser.Easing.Linear.None, true);
    fadeTween.onComplete.add(this.next, this);
    const scaleTween = this.game.add.tween(this.actors.earth.scale).to({
      x: 2,
      y: 2
    }, 3600, Phaser.Easing.Linear.None, true);
    ...
    burstEmitter.makeParticles('earth-boom-sheet', new Array(quant).fill(0).map((v, i) => i), quant, false, false);
    burstEmitter.gravity = 0;
    ...
    const quant2 = 16 * 16; // second boom

    const secondBurstEmitter = game.add.emitter(0, 0, quant2);
    secondBurstEmitter.makeParticles('earth-boom2-sheet', new Array(quant2).fill(0).map((v, i) => i), quant2, false, false);
    ...
    const burst2Tween = this.game.add.tween(secondBurstEmitter).to({
      alpha: 0.0
  ...
  next() {
    this.game.stateTransition.to('ScoreState', false, false, {
      stats: this.stats,
      music: this.sounds.DefeatMusic
    });
  }
  ...
  generateCirclePoints(count = 36, width = 1000) {
    let x, y;
    let angle = 360 / count;
    let circlePoints = []; // generate a list of x y points around a circle
